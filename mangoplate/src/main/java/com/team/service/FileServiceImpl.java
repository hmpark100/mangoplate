...
public class FileServiceImpl {
	 ...
	 */
	/*
	 * public void multiFileSave(MangoRestVO vo, HttpServletRequest request) throws
	 * Exception { //파일을 upload에 저장 for(int i=0; i<vo.getFiles().length; i++) {
	 ...
	 *//**
		 * 식당등록 : 멀티파일 체크
		 *//*
			 * public MangoRestVO multiFileCheck(MangoRestVO vo) { for(CommonsMultipartFile
			 * file : vo.getFiles()) { if(!file.getOriginalFilename().equals("")) { UUID
			 * uuid = UUID.randomUUID();
			 * 
			 * vo.getMfiles().add(file.getOriginalFilename());
			 * vo.getMsfiles().add(uuid+"_"+file.getOriginalFilename()); }else {
			 * vo.getMfiles().add(""); vo.getMsfiles().add(""); } }
			 * vo.setMfile1(vo.getMfiles().get(0)); vo.setMsfile1(vo.getMsfiles().get(0));
			 * 
			 * return vo; }
			 */
